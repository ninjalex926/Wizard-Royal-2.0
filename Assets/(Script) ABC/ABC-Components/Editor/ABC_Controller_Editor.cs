...
//				new Color(211F, 211F, 211F, 0.5F));
	...
	public override void OnInspectorGUI(){
        ...
        {
            // add standard defaults here
            var window = EditorWindow.GetWindow(typeof(ABC_ControllerAbility_EditorWindow), true);
        ...
        {
            // add standard defaults here
            var window = EditorWindow.GetWindow(typeof(ABC_Controller_EditorWindow), true);
        ...
        if (GUILayout.Button(new GUIContent(" Refresh ABC Pool", RefreshIcon), GUILayout.Width(150)))
        {
            abilityCont.CreatePools();
            abilityCont.Reload();
        }
        ...
        ResetLabelWidth();
        EditorGUILayout.BeginHorizontal();
			EditorGUILayout.PropertyField(GetTarget.FindProperty("importAbilitiesObj"), new GUIContent("Import Object"));

			if(GUILayout.Button(new GUIContent(" Import", ImportIcon), GUILayout.Width(100))){
				// start import method
				abilityCont.ImportExportedAbilities();
                //Set dirty so inspector knows we made a change 
                EditorUtility.SetDirty(abilityCont);

            EditorUtility.DisplayDialog("Ability Import", "Ability Import was successfully completed.", "Ok");


        }
        EditorGUILayout.EndHorizontal();

        InspectorHelpBox("Add an ABC Exported Abilities Scriptable Object to import abilites to this entity. ");
