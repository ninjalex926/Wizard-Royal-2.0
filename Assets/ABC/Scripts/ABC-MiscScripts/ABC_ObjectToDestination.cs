...
public class ABC_ObjectToDestination : MonoBehaviour {
    ...
    private void ManuallyDetectEnvironmentCollision() {
        ...
        hitCol = Physics.OverlapSphere(meCollider.bounds.center, (meCollider.bounds.extents.magnitude / 2)).ToList();
        ...
        if (hitCol.Where(c => c.name.Contains("Terrain") || c.name.Contains("Environment") || c.tag == "Environment").Count() > 0) {
            this.DisableScript();
        }
    ...
    private void MoveObjectToDestination() {
        ...
        if (this.secondsToTarget > 0) {
            ...
            // lerp to position
            if (meRigidbody != null) {
                meRigidbody.position = Vector3.Lerp(meStartingPosition, destinationPosition, lerpTime);
            }
            else {
    ...
    void OnEnable() {
        ...
        //Add method call to fixed update if you want to check more often, will decrease performance slightly
        this.InvokeRepeating("ManuallyDetectEnvironmentCollision", 0f, 1f);
    }
