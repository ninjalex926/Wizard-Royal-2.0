...
public class ABC_Ability {
    ...
    public float airAbilityDistanceFromGround = 4f;
    ...
    public bool raycastBlockable = false;
    ...
    public bool auxiliarySoftTarget = false;
    ...
    public bool abilityBeforeTargetIndicatorScaleToEffectRadius = true;

    /// <summary>
    /// If true then all targets will have to be soft target confirmed before the ability will activate
    /// </summary>
    [Tooltip("Soft select will not change target unless confirmed by user")]
    public bool abilityBeforeSoftTarget = false;
    ...
    /// <summary>
    /// If true then caster will rotate to the same direction camera is facing
    /// </summary>
    [Tooltip("If true then caster will rotate to the same direction camera is facing")]
    public bool rotateToCameraCenter = false;
    ...
    /// <summary>
    /// How long to wait before the projectile is released after initiating. Allows time for animation/effect to play out correctly
    /// </summary>
    [Tooltip("How long to wait before the projectile fires. Allows time for animation/effect to play out correctly")]
    ...
    public bool ignorePrepareTimeAdjustments = false;
    ...
    private GameObject scrollAbilityAesthetic {
        ...
        set {
            ...
            if (value != null){
                ...
                if (this.scrollAbilityAestheticAnimator != null && this.scrollAbilityAestheticAnimator.gameObject.GetComponent<ABC_AnimationRunner>() == null) {
                    this.scrollAbilityAestheticAnimationRunner = this.scrollAbilityAestheticAnimator.gameObject.AddComponent<ABC_AnimationRunner>();
                }
                ...
                if (this.scrollAbilityAestheticAnimationRunner == null && this.scrollAbilityAestheticAnimator != null)
                    this.scrollAbilityAestheticAnimationRunner = this.scrollAbilityAestheticAnimationRunner.gameObject.GetComponent<ABC_AnimationRunner>();
    ...
    /// </summary>
    private ABC_AnimationRunner scrollAbilityAestheticAnimationRunner = null;
    ...
    public void ClearObjectPools() {
        ...
        this.effects.ForEach(pool => pool.effectPool.Clear());
    ...
    public ABC_AnimationRunner GetCurrentScrollAbilityAnimationRunner(ABC_IEntity Originator = null)
    {
    ...
    /// </summary>
    /// <param name="Originator">Entity that activated the ability</param>
    /// <param name="IgnoreComboLockResets">(Optional) If true then no combo locks will be reset, else resets will occur as normal (if another </param>
    ...
    public bool IsComboBlocked(ABC_IEntity Originator, bool IgnoreComboLockResets = false) {
        ...
        List<ABC_Ability> Abilities = Originator.AllAbilities;

        int positionInList = 0;

        for (int i = 0; i < Abilities.Count; i++) {
            // if the input of the ability is the same then add to temp list as long as it's enabled, not a scroll ability and its an ability combo
            if ((Abilities[i].key == this.key && this.keyInputType == InputType.Key || Abilities[i].keyButton == this.keyButton && this.keyInputType == InputType.Button)
                && Abilities[i].scrollAbility == false && Abilities[i].abilityCombo == true && Abilities[i].isEnabled() == true) {
        ...
        if (positionInList == 0 && lastComboLocked != null && Time.time - lastComboLocked.comboLockedTime > lastComboLocked.comboNextActivateTime) {
        ...
        if (positionInList == 0 && lastComboLocked == null || lastComboLocked != null && this.comboLocked == false && Time.time - lastComboLocked.comboLockedTime < lastComboLocked.comboNextActivateTime && (lastComboLocked.comboHitRequired == false || lastComboLocked.comboHitRequired == true && lastComboLocked.comboLockHit == true) || tempComboAbilities.Count <= 1) {
            return false;
        }
        else {
            return true;
    ...
    /// </summary>
    /// <remarks>Currently only used to apply effects on objects hit by ability raycasts</remarks>
    ...
    public void ApplyAbilityEffectsToObject(GameObject Obj, ABC_IEntity Originator, GameObject Projectile = null, Vector3 HitPoint = default(Vector3), bool AddedBySplash = false, bool IsEffectLink = false) {
        ...
        //Create new list of effects so we can modify
        List<Effect> effectsToAdd = this.effects.ToList();
        ...
        if (this.enableAbilityEffectLinks) {
            ...
            foreach (int ID in this.effectLinkAbilityIDs.Where(a => a != this.abilityID).ToList())
                 effectsToAdd.AddRange(Originator.GetAbilitiesEffects(ID));
        }
        ...
        iEntity.AddEffectsToEntity(effectsToAdd, Originator, this.abilityType, IsEffectLink, Projectile, Obj, HitPoint, this.name, this.activateAnimationFromHit, this.activateSpecificHitAnimation ? this.hitAnimationToActivate : "", this.overrideWeaponBlocking, this.reduceWeaponBlockDurability, this.overrideWeaponParrying);
    ...
    public GameObject ActivateGraphicAtPosition(AbilityPositionGraphicType GraphicType, Vector3 Position, Quaternion Rotation = default(Quaternion)) {
        ...
        graphicObj.transform.rotation = Rotation;
    ...
    /// <param name="Originator">Entity that activated the ability</param>
    public void DisableProjectile(GameObject AbilityObj, ABC_IEntity Originator) {
        ...
        if (this.abilityType == AbilityType.Melee)
            Originator.RemoveFromActiveMeleeAbilities(this);
    ...
    /// <returns></returns>
    private IEnumerator DestroyObject(GameObject Obj, float Delay = 0f) {
    ...
    private IEnumerator ActivateAbility(ABC_IEntity Originator) {
        ...
        if (this.tempAbilityActivationIntervalAdjustment > 0f)
            Originator.TemporarilyAdjustAbilityActivationInterval(this.tempAbilityActivationIntervalAdjustment);
        ...
        if (this.abilityType == AbilityType.Melee && this.AbilityActivationInterrupted() == false)
            Originator.AddToActiveMeleeAbilities(this);
    ...
    private GameObject CreateAbilityEndObjects(bool CreateOne = false) {
        ...
        if (this.useAbilityEndAesthetics == true && this.abilityEndGraphic.GameObject != null && this.abilityEnabled == true) {
            ...
            for (int i = 0; i < objCount; i++) {
                ...
                // copy child object for additional Aesthetic 
                if (this.abilityEndSubGraphic.GameObject != null) {
    ...
    private void CreateEffectObjects() {
        ...
        if (this.abilityEnabled == true)
            this.effects.ForEach(effect => effect.CreateEffectObjects());
    ...
    /// <returns>Will return the graphic gameobject which has been created</returns>
    private GameObject ActivateGraphic(ABC_IEntity Originator, AbilityGraphicType GraphicType, bool IgnoreDelay = false) {
        ...
        ABC_Utilities.mbSurrogate.StartCoroutine(EnableGraphic(graphicObj, Originator, GraphicType, IgnoreDelay));
    ...
    /// <param name="GraphicType">Type of graphic to activate: Preparing, Initiating, Reloading, AbilityActivation</param>
    /// <param name="IgnoreDelay">If true then any delays setup will be ignored and graphic will activate instantly</param>
    private IEnumerator EnableGraphic(GameObject GraphicObj, ABC_IEntity Originator, AbilityGraphicType GraphicType, bool IgnoreDelay = false) {
        ...
        switch (GraphicType) {
            ...
            case AbilityGraphicType.Initiating:
                ...
                duration = this.initiatingAestheticDuration;
                delay = this.initiatingAestheticDelay;
        ...
        switch (startingPosition) {
            ...
            case StartingPosition.OnSelfTag:
                GameObject onSelfTagObj = ABC_Utilities.TraverseObjectForTag(Originator.gameObject, positionOnTag);
    ...
    /// <summary>
    /// Will activate/deactivate the world target indicator which places the entities indicator graphic at the point provided
    /// </summary>
    /// <param name="Enabled">True if the indicator should show, else false</param>
    /// <param name="Point">Vector3 Position where indicator will show</param>
    /// <param name="Originator">Entity that activated the ability</param>
    private void ActivateWorldTargetIndicator(bool Enabled, Vector3 Point, ABC_IEntity Originator) {

        GameObject abilityWorldTargetIndicator = Originator.abilityWorldTargetIndicator;
        GameObject abilityWorldTargetOutRangeIndicator = Originator.abilityWorldTargetOutRangeIndicator;


        if (Enabled == true && abilityWorldTargetIndicator != null && Point != Vector3.zero) {


            // if we have an out range indicator and range is on then we care if were in range or not 
            if (abilityWorldTargetOutRangeIndicator != null && this.useRange == true) {

                Vector3 rangeSpawnPos;

                // if on world then we need to pretend that the range is coming from the caster, else just do range from spawn position 
                rangeSpawnPos = Originator.transform.position;

                //Calculate distance
                float distance = Vector3.Distance(rangeSpawnPos, Point);
                    ...
                    if (distance >= this.selectedTargetRangeGreaterThan && distance <= this.selectedTargetRangeLessThan) {
                        ...
                        // turn out of range indicator off 
                        if (abilityWorldTargetOutRangeIndicator != null) {
                        abilityWorldTargetOutRangeIndicator.SetActive(false);
                    }

                    // turn on in range indicator and position it 
                    abilityWorldTargetIndicator.transform.position = Point + new Vector3(0, 0.1f, 0);
                    abilityWorldTargetIndicator.SetActive(true);

                    // set the scale to the scale of projectile if setting has been set

                    if (this.abilityBeforeTargetIndicatorScaleToEffectRadius == true) {
                        abilityWorldTargetIndicator.transform.localScale = this.effectRangeLocalScale;
                    }
                    else {
                        abilityWorldTargetIndicator.transform.localScale = Originator.abilityWorldTargetIndicatorDefaultSize;
                    }
                    ...
                    // turn off in range indicator

                    abilityWorldTargetIndicator.SetActive(false);

                    // turn on out of range indicator 

                    abilityWorldTargetOutRangeIndicator.transform.position = Point + new Vector3(0, 0.1f, 0);
                    abilityWorldTargetOutRangeIndicator.SetActive(true);

                    if (this.abilityBeforeTargetIndicatorScaleToEffectRadius == true) {
                        abilityWorldTargetOutRangeIndicator.transform.localScale = this.effectRangeLocalScale;
                    }
                    else {

                        abilityWorldTargetOutRangeIndicator.transform.localScale = Originator.abilityWorldTargetOutRangeIndicatorDefaultSize;
                    }
                ...
                // we don't have an off range indicator so just always show this one if were in range or not 
        ...
        else if (Enabled == false && abilityWorldTargetIndicator != null) {

            // were not raycasting to anything (false was put in method) so just turn it off 
            abilityWorldTargetIndicator.SetActive(false);
    ...
    private void StartAnimationRunner(AbilityAnimationState State, ABC_AnimationRunner AnimationRunner){
    ...
    private void EndAnimationRunner(AbilityAnimationState State, ABC_AnimationRunner AnimationRunner, float Delay = 0f)   {
    ...
    private bool AbilityActivationInterrupted() {
    ...
    private bool TargetExists(ABC_IEntity Originator) {
       ...
       //If ability is a ray cast but not set to: forward, selected target, nearest tag crosshair, mouse forward, mouse 2d or mouse target then we will default to forward
        if (this.abilityType == AbilityType.RayCast && (this.travelType != TravelType.SelectedTarget && this.travelType != TravelType.NearestTag && this.travelType != TravelType.Crosshair && this.travelType != TravelType.Forward && this.travelType != TravelType.MouseForward && this.travelType != TravelType.MouseTarget && this.travelType != TravelType.Mouse2D))
            this.travelType = TravelType.Forward;
        ...
        if (this.travelType == TravelType.NearestTag && this.noTargetStillTravel == false) {
            ...
            if (this.GetObjectWithNearestTag(this.GetStartingPosition(Originator, AbilityStartPositionType.StartPosition), this.travelNearestTagRange, this.travelNearestTagList, IgnoreEntity) == null) {
    ...
    private bool CorrectElevation(ABC_IEntity Originator) {
        ...
        switch (this.LandOrAir) {
            ...
            case AbilityLandOrAir.Air:
                ...
                if (OriginatorsDistanceFromGround(Originator) >= this.airAbilityDistanceFromGround) {
                    return true;
                }
                else {
                    return false;
            ...
            case AbilityLandOrAir.Land:
                ...
                if (OriginatorsDistanceFromGround(Originator) <= 1) {
                    return true;
                }
                else {
                    return false;
    ...
    private void ResetAllComboLocks(ABC_IEntity Originator, bool BypassResetRestriction = false) {
        ...
        foreach (ABC_Ability item in Originator.AllAbilities.Where(item => item.abilityCombo == true)) {
    ...
    /// <summary>
    /// Returns how far up the originator entity is from the closest object below (jumping/in air)
    /// </summary>
    /// <param name="Originator">Entity that activated the ability</param>
    /// <returns>Float distance from ground</returns>
    private float OriginatorsDistanceFromGround(ABC_IEntity Originator) {


        // raycast down vector
        Vector3 dir = new Vector3(0, -1, 0);

        // what we hit with our rays
        RaycastHit hit;

        //edit: to draw ray also//
        //Debug.DrawRay(transform.position,dir*DistanceToGround,Color.green);
        //end edit//

        if (Physics.Raycast(Originator.transform.position, dir, out hit, 100f)) {
    ...
    private void RotateOriginatorToTarget(ABC_IEntity Originator) {
        ...
        if (this.rotateToSelectedTarget || this.travelType == TravelType.MouseForward) {
            switch (this.travelType) {
                ...
                case TravelType.Crosshair:
                    Originator.LookAt(rayCastTargetPosition);
                    break;
        ...
        //if rotate to camera center is true and it's a melee attack or a forward travel then rotate to same direction camera is facing
        if (this.rotateToCameraCenter == true) {

            if (this.abilityType == AbilityType.Melee || this.travelType == TravelType.Forward) {
                ...
                if (cameraDirection.sqrMagnitude != 0.0f) {
                    ...
                    Originator.LookAt(Originator.transform.position + cameraDirection);
                }
            }
    ...
    private IEnumerator ToggleOriginatorsMovement(ABC_IEntity Originator, bool Enable = true, bool FreezePosition = true, bool DisableComponents = false, float Duration = 0f) {
        ...
        float functionRequestTime = Time.realtimeSinceStartup;
    ...
    private IEnumerator DefyOriginatorGravity(ABC_IEntity Originator, float Duration, bool RaiseEvent = false) {

        //stop any velocity first 
        Originator.StopVelocity();

        // turn gravity off

        Originator.UseGravity(false);
    ...
    private bool OriginatorInRangeOfTarget(ABC_IEntity Originator) {
        ...
        //If we not using range or the travel type isn't selected target, to world or self and the ability isn't a melee, or is a melee but will always still 'travel' then return true as we don't care for range
        if (this.useRange == false || this.travelType != TravelType.SelectedTarget && this.travelType != TravelType.ToWorld && this.travelType != TravelType.Self && (this.abilityType != AbilityType.Melee || this.abilityType == AbilityType.Melee && this.target == null && this.noTargetStillTravel == true)) {
            // were not ranging so we don't care if in range so we return true
            return true;
        }
        else {
            ...
            if (this.startingPosition == StartingPosition.Target) {
                rangeSpawnPos = this.target.transform.position;
            }
            else if (this.startingPosition == StartingPosition.OnWorld) {
                rangeSpawnPos = this.worldTargetPosition; 
            }
            else {
    ...
    private bool OriginatorFacingTarget(ABC_IEntity Originator) {
        ...
        if (this.targetFacing == true && (this.travelType == TravelType.SelectedTarget || this.travelType == TravelType.NearestTag || this.abilityType == AbilityType.Melee) && this.target.gameObject != Originator.transform.gameObject
            || this.targetFacing == true && this.travelType == TravelType.Self && this.startingPosition != StartingPosition.Self && this.startingPositionOnObject.GameObject.transform.IsChildOf(Originator.transform) == false) {
    ...
    /// </summary>
    /// <param name="OriginatorsPosition">Position of entity that activated the ability</param>
    ...
    private bool CanBeUsedAsSurroundingTarget(Vector3 OriginatorsPosition, GameObject Target) {
        ...
        if (iEntity.HasABCStateManager() == false || iEntity.blockSurroundingObjectStatus == true || Vector3.Distance(OriginatorsPosition, Target.transform.position) > this.surroundingObjectTargetRange)
            return false;
        ...
        if (this.surroundingObjectTargetRestrict == true) {
            ...
            if (ABC_Utilities.ObjectHasTag(Target, surroundingObjectTargetAffectTag)) {
                return true;
            }
            else {
    ...
    private GameObject GetSurroundingTarget(ABC_IEntity originator) {
        ...
        if (this.surroundingObjectTarget == true && originator.target != null && this.CanBeUsedAsSurroundingTarget(originator.transform.position, originator.target)) {
            //If we can then it is set to the global variable used later in the controller
            retVal = originator.target;

        }
        else if (this.surroundingObjectTarget == true && this.surroundingObjectAuxiliarySoftTarget == true && originator.softTarget != null && this.CanBeUsedAsSurroundingTarget(originator.transform.position, originator.softTarget)) {
    ...
    private List<GameObject> GetSurroundingTags(ABC_IEntity Originator) {
        ...
        foreach (ABC_IEntity entity in ABC_Utilities.GetAllABCEntitiesInRange(GetStartingPosition(Originator, AbilityStartPositionType.StartPosition), this.surroundingObjectTagsRange).ToList()) {
            ...
            if (ABC_Utilities.ObjectHasTag(entity.gameObject, surroundingObjectTagAffectTag)) {
    ...
    void SendProjectileToStartPosition(ABC_IEntity Originator, GameObject StartPositionObject, Vector3 StartingPositionOffset, float StartingPositionForwardOffset, float StartingPositionRightOffset) {
    ...
    private Vector3 GetMousePosition(ABC_IEntity Originator) {
        ...
        Ray ray = Originator.Camera.ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;
    ...
    private IEnumerator WaitForNewOriginatorTarget(ABC_IEntity Originator) {
        ...
        if (this.travelType == TravelType.SelectedTarget || this.startingPosition == StartingPosition.Target) {
            ...
            // if were doing softtarget then turn override on (we also check the ability has the "ability before target" option on and its not the override)
            if (this.abilityBeforeSoftTarget == true && this.abilityBeforeTarget == true) {

                // waiting on target with softtargetoverride set to true (means players has to confirm new target)
                Originator.SelectNewTarget(TargetType.Target, true);
            }
            else {
                // waiting on target 
            ...
            while (Originator.NewTargetSelected(TargetType.Target) == false) {
        ...
        if (this.travelType == TravelType.MouseTarget || this.travelType == TravelType.Mouse2D) {
            ...
            while (Originator.NewTargetSelected(TargetType.Mouse) == false) {
                yield return null;
            }
        ...
        if (this.travelType == TravelType.ToWorld || this.startingPosition == StartingPosition.OnWorld) {
            ...
            while ((Originator.NewTargetSelected(TargetType.Target) == false || Originator.NewTargetSelected(TargetType.Mouse) == false || Originator.NewTargetSelected(TargetType.World) == false) && this.AbilityActivationInterrupted() == false) {
                // whilst waiting on new target if were starting or travelling to world show a graphic to help the user 
                if (this.travelType == TravelType.ToWorld || this.startingPosition == StartingPosition.OnWorld) {

                    Ray ray = new Ray();
                    // change hit position depending on target select type 
                    if (Originator.targetSelectType == TargetSelectType.Mouse) {
                        ray = Originator.Camera.ScreenPointToRay(Input.mousePosition);
                    }
                    ...
                    if (Physics.Raycast(ray, out hit)) {
                        this.ActivateWorldTargetIndicator(true, hit.point, Originator);
                    }
                    else {
                        this.ActivateWorldTargetIndicator(false, Vector3.zero, Originator);
                    }
            ...
            // make sure ability before target indicator is turned off now we no longer waiting 
            this.ActivateWorldTargetIndicator(false, Vector3.zero, Originator);
        }
    ...
    private bool SetAbilityTargets(ABC_IEntity Originator) {
        ...
        if (this.travelType == TravelType.NearestTag) {
            ...
            //Find object
            target = this.GetObjectWithNearestTag(this.GetStartingPosition(Originator, AbilityStartPositionType.StartPosition), this.travelNearestTagRange, this.travelNearestTagList, IgnoreEntity, this.travelNearestTagRandomiseSearch);
        ...
        if (this.abilityType == AbilityType.Melee || this.travelType == TravelType.SelectedTarget || this.startingPosition == StartingPosition.Target || this.useProjectileToStartPosition == true && this.projToStartStartingPosition == StartingPosition.Target) {
            ...
            if (target != null && this.selectedTargetRestrictTargets == true && ABC_Utilities.ObjectHasTag(target, this.selectedTargetOnlyCastOnTag) == false) {
            ...
            if (target == null && this.abilityType != AbilityType.Melee)
                return false;
    ...
    private IEnumerator StartPreparing(ABC_IEntity Originator) {
        ...
        if (this.ignorePrepareTimeAdjustments == false)
        ...
        if (this.useProjectileToStartPosition == true && (this.projToStartPosGraphic.GameObject != null || this.useOriginalProjectilePTS == true)) {
            ...
            this.SendProjectileToStartPosition(Originator, this.GetStartingPositionObject(Originator, AbilityStartPositionType.StartPosition), this.startingPositionOffset, this.startingPositionForwardOffset, this.startingPositionRightOffset);
            ...
            foreach (AdditionalStartingPosition addStartPosition in this.additionalStartingPositions)
                this.SendProjectileToStartPosition(Originator, this.GetStartingPositionObject(Originator, AbilityStartPositionType.AdditionalStartPosition, addStartPosition), addStartPosition.startingPositionOffset, addStartPosition.startingPositionForwardOffset, addStartPosition.startingPositionRightOffset);
        }
        ...
        if (abilityPrepareTime > 0) {
            ...
            if (this.usePreparingAesthetics == true) {
                ...
                if (this.stopMovementOnPreparing)
                    ABC_Utilities.mbSurrogate.StartCoroutine(this.ToggleOriginatorsMovement(Originator, false, this.stopMovementOnPreparingFreezePosition, stopMovementOnPreparingDisableComponents, this.stopMovementOnPreparingDuration));

                // If set then raise the stop originator movement event
                if (this.stopMovementOnPreparingRaiseEvent)
                    ABC_Utilities.mbSurrogate.StartCoroutine(this.RaiseOriginatorsToggleMovementEvent(Originator, false, this.stopMovementOnPreparingDuration));
                ...
                if (this.defyGravityPreparing == true && this.defyGravityPreparingDuration > 0) {
                    ...
                    ABC_Utilities.mbSurrogate.StartCoroutine(DefyOriginatorGravity(Originator, this.defyGravityPreparingDuration, this.defyGravityPreparingRaiseEvent));
                ...
                if (this.moveSelfWhenPreparing == true)
                    ABC_Utilities.mbSurrogate.StartCoroutine(Originator.MoveSelfByOffset(this.moveSelfPreparingOffset, this.moveSelfPreparingForwardOffset, this.moveSelfPreparingRightOffset, this.moveSelfPreparingDuration, this.moveSelfPreparingDelay));
                ...
                if (this.moveSelfToTargetWhenPreparing == true) {
                    ...
                    if (moveSelfTarget != null && (Vector3.Distance(Originator.transform.position, moveSelfTarget.transform.position) > this.moveSelfToTargetPreparingStopDistance + 0.5f)) {
                        ABC_Utilities.mbSurrogate.StartCoroutine(Originator.MoveSelfToObject(moveSelfTarget, this.moveSelfToTargetPreparingStopDistance, this.moveSelfToTargetPreparingOffset, this.moveSelfToTargetPreparingForwardOffset, this.moveSelfToTargetPreparingRightOffset, this.moveSelfToTargetPreparingDuration, this.moveSelfToTargetPreparingDelay));
                    } else {
    ...
    private IEnumerator ActivateInitiatingAesthetics(ABC_IEntity Originator) {
        ...
        if (this.useInitiatingAesthetics == true && this.AbilityActivationInterrupted() == false) {
           Originator.AddToDiagnosticLog("Initiating " + this.name);
            ...
            // stop originator from movement when initiating
            if (this.stopMovementOnInitiate)
                ABC_Utilities.mbSurrogate.StartCoroutine(this.ToggleOriginatorsMovement(Originator, false, this.stopMovementOnInitiateFreezePosition, this.stopMovementOnInitiateDisableComponents, this.stopMovementOnInitiateDuration));

            // If set then raise the stop originator movement event
            if (this.stopMovementOnInitiateRaiseEvent)
                ABC_Utilities.mbSurrogate.StartCoroutine(this.RaiseOriginatorsToggleMovementEvent(Originator, false, this.stopMovementOnInitiateDuration));
            ...
            if (this.defyGravityInitiating == true && this.defyGravityInitiatingDuration > 0) {
                ...
                ABC_Utilities.mbSurrogate.StopCoroutine("DefyOriginatorGravity");
                ABC_Utilities.mbSurrogate.StartCoroutine(DefyOriginatorGravity(Originator, this.defyGravityInitiatingDuration, this.defyGravityInitiatingRaiseEvent));
            ...
            if (this.moveSelfWhenInitiating == true)
                ABC_Utilities.mbSurrogate.StartCoroutine(Originator.MoveSelfByOffset(this.moveSelfInitiatingOffset, this.moveSelfInitiatingForwardOffset, this.moveSelfInitiatingRightOffset, this.moveSelfInitiatingDuration, this.moveSelfInitiatingDelay));
            ...
            if (this.moveSelfToTargetWhenInitiating == true) {
                ...
                //if further away then stop distance and we have a target then move to it 
                if (moveSelfTarget != null && (Vector3.Distance(Originator.transform.position, moveSelfTarget.transform.position) > this.moveSelfToTargetInitiatingStopDistance + 0.5f))
                    ABC_Utilities.mbSurrogate.StartCoroutine(Originator.MoveSelfToObject(moveSelfTarget, this.moveSelfToTargetInitiatingStopDistance, this.moveSelfToTargetInitiatingOffset, this.moveSelfToTargetInitiatingForwardOffset, this.moveSelfToTargetInitiatingRightOffset, this.moveSelfToTargetInitiatingDuration, this.moveSelfToTargetInitiatingDelay));

            } 


            // get object and particle if they exist
            initiatingObj = ActivateGraphic(Originator, AbilityGraphicType.Initiating);

            //Start animation on ABC runner 
            if (this.initiatingAnimationRunnerOnEntity)
                this.StartAnimationRunner(AbilityAnimationState.Initiate, Originator.animationRunner);

            if (this.initiatingAnimationRunnerOnScrollGraphic)
                this.StartAnimationRunner(AbilityAnimationState.Initiate, GetCurrentScrollAbilityAnimationRunner(Originator));

            if (this.initiatingAnimationRunnerOnWeapon)
                Originator.GetCurrentEquippedWeaponAnimationRunners().ForEach(ar => this.StartAnimationRunner(AbilityAnimationState.Initiate, ar));

            //Start animation on animator 
            if (this.initiatingAnimateOnEntity)
                this.StartAnimation(AbilityAnimationState.Initiate, Originator.animator);

            //If enabled then activate the animation on the graphic object
            if (this.initiatingAnimateOnScrollGraphic)
                this.StartAnimation(AbilityAnimationState.Initiate, GetCurrentScrollAbilityAnimator(Originator));

            if (this.initiatingAnimateOnWeapon)
                Originator.GetCurrentEquippedWeaponAnimators().ForEach(a => this.StartAnimation(AbilityAnimationState.Initiate, a));
            ...
            if (this.abilityToggle == AbilityToggle.Off || this.abilityToggle != AbilityToggle.Off && (this.canCastWhenToggled == true || this.repeatInitiatingAnimationWhilstToggled == false)) {
                ...
                //Loop through to cancel animations early if ability interuptted 
                for (var i = maxAnimationDuration; i > 0; i = i - 0.2f) {
                ...
                // cancel the stop movement if we did not use a duration (else it would have been stopped earlier with the duration) unless the ability has been interrupted 
                if (this.stopMovementOnInitiate && (this.stopMovementOnInitiateDuration == 0f || this.AbilityActivationInterrupted()))
                    ABC_Utilities.mbSurrogate.StartCoroutine(this.ToggleOriginatorsMovement(Originator, true, this.stopMovementOnInitiateFreezePosition, stopMovementOnInitiateDisableComponents));


                // If set then raise then enable originator movement event if we did not use a duration (else it would have been stopped earlier with the duration) unless the ability has been interrupted 
                if (this.stopMovementOnInitiateRaiseEvent && (this.stopMovementOnInitiateDuration == 0f || this.AbilityActivationInterrupted()))
                    ABC_Utilities.mbSurrogate.StartCoroutine(this.RaiseOriginatorsToggleMovementEvent(Originator, true));
        ...
        if (this.AbilityActivationInterrupted()) {
            ...
            if (initiatingObj != null)
                ABC_Utilities.mbSurrogate.StartCoroutine(DestroyObject(initiatingObj));
    ...
    private IEnumerator InitiateAbility(ABC_IEntity Originator) {
        ...
        if (this.modifyGameSpeedOnInitiation == true)
        ...
        // wait before firing ability
        if (this.delayBetweenInitiatingAndProjectile < 0.3) {
            // if the delay is between 0-0.2 we don't need to check in a loop if casting was interuppted as its almost impossible.

            yield return new WaitForSeconds(this.delayBetweenInitiatingAndProjectile);

        }
        else {
            for (var i = this.delayBetweenInitiatingAndProjectile; i > 0; i--) {
    ...
    private void InitiateRayCast(ABC_IEntity Originator) {
        ...
        if (this.travelType == TravelType.Crosshair) {
            ...
            ray = Originator.Camera.ScreenPointToRay(Input.mousePosition + this.selectedTargetOffset + this.GetMissChancePositionOffset(Originator));
        ...
        // ray cast out 
        RaycastHit[] hit = Physics.SphereCastAll(ray, this.rayCastRadius, this.rayCastLength, this.affectLayer);
        ...
        hit = hit.OrderBy(x => Vector3.Distance(ray.origin, x.transform.position)).ToArray();
        ...
        for (int i = 0; i < hit.Length; i++) {
            ...
            if (hit[i].transform.IsChildOf(Originator.transform) || this.raycastIgnoreTerrain == true && Terrain.activeTerrain != null && hit[i].transform.gameObject == Terrain.activeTerrain.transform.gameObject ||  hit[i].transform.gameObject.name.Contains("ABC*_"))
                continue;
            ...
            this.ActivateGraphicAtPosition(AbilityPositionGraphicType.End, hit[i].point);

            // check for state manager script (we only effect these types of objects)
            ABC_IEntity iEntity = ABC_Utilities.GetStaticABCEntity(hit[i].transform.gameObject);      
            ...
            if (abilityIgnoreTag.Count > 0 &&  ABC_Utilities.ObjectHasTag(hit[i].transform.gameObject, abilityIgnoreTag) == true)
                continue;

            // if the object hit does not have a tag we require then continue
            if (abilityRequiredTag.Count > 0 && ABC_Utilities.ObjectHasTag(hit[i].transform.gameObject, abilityRequiredTag) == false)
                continue;
        ...
        foreach (int rayCastIndex in hits) {
            ...
            //Set combo lock
            this.SetComboLock(hit[rayCastIndex].collider.gameObject);

           Originator.AddToDiagnosticLog(this.name + " raycast hit: " + hit[rayCastIndex].transform.name);
            // run effects and states 
            ApplyAbilityEffectsToObject(hit[rayCastIndex].collider.gameObject, Originator, null, hit[rayCastIndex].point);
        }
    ...
    private IEnumerator InitiateProjectile(ABC_IEntity Originator) {
        ...
        //Setup new list of additional starting positions
        List<Vector3> extraStartingPositions = new List<Vector3>();
       ...
       || this.travelType == TravelType.MouseTarget || this.travelType == TravelType.Mouse2D || this.travelType == TravelType.Crosshair)) {
            ...
            startingPosition = GetTargetPosition(Originator);

            //If we have any additional starting positions defined then we will add the target starting position for each one
            foreach (AdditionalStartingPosition addStartPosition in this.additionalStartingPositions) {
                extraStartingPositions.Add(startingPosition);
            }

        } else {
            // start at normal starting position
            startingPosition = GetStartingPosition(Originator, AbilityStartPositionType.StartPosition);

            //Retrieve all the additional starting positions
            foreach (AdditionalStartingPosition addStartPosition in this.additionalStartingPositions) {
                extraStartingPositions.Add(GetStartingPosition(Originator, AbilityStartPositionType.AdditionalStartPosition, addStartPosition));
            }
       ...
       yield return ABC_Utilities.mbSurrogate.StartCoroutine(this.CreateProjectile(Originator, startingPosition, this.startingPositionOffset, this.startingPositionForwardOffset, this.startingPositionRightOffset, this.GetStartingPositionObject(Originator, AbilityStartPositionType.StartPosition), this.startingRotation));

        //Create additional projectiles 
        for (int i = 0; i < this.additionalStartingPositions.Count(); i++)  {
          yield return  ABC_Utilities.mbSurrogate.StartCoroutine(this.CreateProjectile(Originator, extraStartingPositions[i], this.additionalStartingPositions[i].startingPositionOffset, this.additionalStartingPositions[i].startingPositionForwardOffset, this.additionalStartingPositions[i].startingPositionRightOffset, this.GetStartingPositionObject(Originator, AbilityStartPositionType.AdditionalStartPosition, this.additionalStartingPositions[i]), this.additionalStartingPositions[i].startingRotation, this.additionalStartingPositions[i].startingDelay));
        }
    ...
    /// <param name="Delay">Delay before ability is created</param>
    private IEnumerator CreateProjectile(ABC_IEntity Originator, Vector3 StartingPosition, Vector3 StartingPositionOffset, float StartingPositionForwardOffset, float StartingPositionRightOffset, GameObject StartingPositionObject,  Vector3 StartingRotation,  float Delay = 0f) {
        ...
        //wait for delay once everything is setup, after delay activate scripts and let it go!
        if (Delay > 0)
            yield return new WaitForSeconds(Delay);
