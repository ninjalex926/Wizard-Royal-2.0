...
public class DisplayController : MonoBehaviour {
    ...
    private bool Triggered() {
        ...
        if (triggerType == TriggerType.Key && Input.GetKeyDown(this.triggerKey))
            return true;


        // if input type is button and the configured button is being pressed return true
        if (triggerType == TriggerType.Button && Input.GetButton(this.triggerButton))
            return true;
    ...
    private void QuitGameManager() {
        ...
        if (this.QuitGameEnabled && this.displayObject.activeInHierarchy && Input.GetKeyDown(this.QuitKey))
            Application.Quit();
