...
public class ABC_StateManager : MonoBehaviour {
    ...
    public class TargeterLimitation {
        ...
        public bool AddPriorityTargeter(GameObject TargeterObj) {

            //If object provided doesn't have the tag we are expecting in the limit then return false
            if (ABC_Utilities.ObjectHasTag(TargeterObj, this.targeterTags) == false)
                return false;
        ...
        public bool AddTargeter(GameObject TargeterObj) {
	...
	public class HealthGUI{
        ...
        /// </summary>
        public float healthOverTimeSliderUpdateDelay = 0.5f;
    ...
    public class HitAnimation {
        ...
        public ABC_AnimationClipReference hitAnimationRunnerClip;
        ...
        /// <summary>
        /// If false then the hit animation will not interrupt any animation currently playing in the animation runner
        /// </summary>
        [Tooltip("If false then the hit animation will not interrupt any animation currently playing in the animation runner")]
        public bool hitAnimationRunnerInterruptCurrentAnimation = true; 
        ...
        private void StartAndStopAnimationRunner(ABC_AnimationRunner AnimationRunner) {


            // if animator parameter is null or animation runner is not given then animation can't start so end here. 
            if (this.hitAnimationRunnerClip.AnimationClip == null || AnimationRunner == null)
                return;

            //Start animation
            AnimationRunner.PlayAnimation(this.hitAnimationRunnerClip.AnimationClip, this.hitAnimationRunnerClipDelay, this.hitAnimationRunnerClipSpeed, this.hitAnimationRunnerClipDuration, this.hitAnimationRunnerInterruptCurrentAnimation);
        ...
        private void StartAnimation(Animator Animator) {
            ...
            AnimatorParameterType animatorParameterType = hitAnimationAnimatorParameterType;
            string animatorParameter = this.hitAnimationAnimatorParameter;
            string animatorOnValue = hitAnimationAnimatorOnValue;
        ...
        /// <summary>
        /// Ends the hit animation
        /// </summary>
        /// <param name="Animator">Animator component</param>
        /// <param name="Delay">Time till animation is stopped</param>
        private IEnumerator EndAnimation(Animator Animator, float Delay) {

            yield return new WaitForSeconds(Delay);

            // set variables to be used later 
            AnimatorParameterType animatorParameterType = hitAnimationAnimatorParameterType;
            string animatorParameter = hitAnimationAnimatorParameter;
            string animatorOffValue = hitAnimationAnimatorOffValue;
        ...
        public HitAnimation(int ID) {
            ...
            this.hitAnimationRunnerInterruptCurrentAnimation = true;
    }
        ...
        /// </summary>
        /// <param name="Animator">Animator of the entity to play the hit animation on</param>
        /// <param name="AnimationRunner">The ABC Animation Runner component to manage the animation clip</param>
        public void ActivateAnimation(Animator Animator, ABC_AnimationRunner AnimationRunner) {

            //Start Animator animations
            this.StartAnimation(Animator);

            ABC_Utilities.mbSurrogate.StartCoroutine(this.EndAnimation(Animator, this.hitAnimationAnimatorDuration));

            //Start and stop animation using the runner
            this.StartAndStopAnimationRunner(AnimationRunner);
        }
    ...
    public bool CanBeTargetedByObject(GameObject Targeter) {
        ...
        foreach (TargeterLimitation TL in this.TargeterLimitations.Where(t => t.enableTargeterLimit == true && ABC_Utilities.ObjectHasTag(Targeter, t.targeterTags))) {
    ...
    public void AddObjectAsPriorityTargeter(GameObject Targeter) {
        ...
        foreach (TargeterLimitation TL in this.TargeterLimitations.Where(t => t.enableTargeterLimit == true)) {
            if (TL.AddPriorityTargeter(Targeter))
                return;
        }
    ...
    public IEnumerator DisplayTextGraphic(string Text, float Duration, Color Color, ABC_IEntity CastingOriginator) {
        ...
        // take effect text out of ABCPool
    ...
    public void AddEffects(List<Effect> effects, ABC_IEntity AfflictingOriginator, AbilityType AbilityType, bool IsEffectLink, GameObject Projectile = null, GameObject ObjectHit = null, Vector3 AbilityHitPoint = default(Vector3), string AbilityName = "", bool ActivateAnimationFromHit = true, string SpecificAnimationToActivate = "", bool OverrideWeaponBlocking = false, bool ReduceWeaponBlockDurability = true, bool OverrideWeaponParrying = false) {
        ...
        //if set to run hit animations on hit (if effect protection is on then we will not activate the hit animation
        if (this.activateHitAnimationsFromAbilityHit == true && ActivateAnimationFromHit == true && blockSuccessful == false) {

            //If no specific animation has been passed through to activate then activate one randomly (depending on probability checks and if configured to activate on hits)
            if (string.IsNullOrEmpty(SpecificAnimationToActivate))
                this.ActivateHitAnimation();
            else
                this.ActivateHitAnimation(SpecificAnimationToActivate, true); // else activate the animation provided
        }
        ...
        StartCoroutine(ActivateEffects(effects, AfflictingOriginator, AbilityType, Projectile, ObjectHit, AbilityHitPoint, AbilityName));
    ...
    public bool IsEffectActive(Effect Effect, ABC_IEntity CastingOriginator) {

        if (ActiveEffects.ContainsKey(CastingOriginator) && ActiveEffects[CastingOriginator].Any(c => c.effect == Effect)) {
            return true;
        } else {
            return false;
        }
    ...
    public bool IsEffectActive(List<string> EffectList) {
        ...
        foreach (string condition in EffectList) {
            ...
            if (this.ActiveEffects.SelectMany(c => c.Value).Select(v => v).Select(e => e.effect.abilityName).Contains(condition)) {
                return true;
    ...
    public float GetStatValue(string StatName, ABCIntegrationType IntegrationType = ABCIntegrationType.ABC) {
        ...
        if (IntegrationType == ABCIntegrationType.GameCreator)       
            return meEntity.GetGCStatValue(StatName);
    ...
    private IEnumerator ZeroHealthManager() {
        ...
        //If zerohealth was already reached then end here
        ...
        this.StartAnimation(StateManagerAnimationState.ZeroHealth, Ani);
        this.StartAnimationRunner(StateManagerAnimationState.ZeroHealth, meEntity.animationRunner);


        //wait for duration then end animation for both animator and animation runner 
        this.EndAnimationRunner(StateManagerAnimationState.ZeroHealth, meEntity.animationRunner);

        yield return new WaitForSeconds(this.zeroHealthAnimationAnimatorDuration);

        this.EndAnimation(StateManagerAnimationState.ZeroHealth, Ani);
    ...
    /// <param name="AnimationRunner">The ABC Animation Runner component to manage the animation clip</param>
    private void StartAnimationRunner(StateManagerAnimationState State, ABC_AnimationRunner AnimationRunner){
    ...
    private void EndAnimationRunner(StateManagerAnimationState State, ABC_AnimationRunner AnimationRunner, float Delay = 0f){
    ...
    private void ActivateHitAnimation() {
        ...
        selectedHitAnimation.ActivateAnimation(Ani, meEntity.animationRunner);
    ...
    /// Function will cycle through all the enabled animations activating the first one that can be activated (depending on dice rolls)
    /// </remarks>
    private void ActivateHitAnimation(string HitAnimationName, bool PlayRandomAnimationIfNotFound = false) {
        ...
        //activate the selected animation
        enabledHitAnimations.ActivateAnimation(Ani, meEntity.animationRunner);
    ...
    private int CountDuplicateActiveEffect(Effect Effect, ABC_IEntity Originator) {
        ...
        if (ActiveEffects.ContainsKey(Originator)){
            ...
            retVal = ActiveEffects[Originator].Where(c => c.effect == Effect).Count();
    ...
    private bool EffectCanBeAdded(Effect Effect, ABC_IEntity Originator, GameObject ObjectHit = null) {

        //Check if effect can activate 
        if (Effect.CanActivate(meEntity, ObjectHit) == false)
            return false;
        ...
        if (IsEffectActive(Effect, Originator) == false) {
            return true;
        } else {
            return false; 
        }
    ...
    /// <param name="AbilityHitPoint">(Optional) The vector position where the ability collided, if setup correctly this will play the effect graphics at that position</param>
    /// <param name="AbilityName">(Optional) The name of the ability which applied the effect (can be used later on to retrieve effects by ability name to dispel etc)</param> 
    private IEnumerator ActivateEffects(List<Effect> Effects, ABC_IEntity AfflictingOriginator, AbilityType AbilityType, GameObject Projectile = null, GameObject ObjectHit = null, Vector3 AbilityHitPoint = default(Vector3), string AbilityName = "") {
        ...
        foreach (Effect effect in Effects) {
            ...
            yield return StartCoroutine(effect.WaitForActivationDelay());
            ...
            if (EffectCanBeAdded(effect, AfflictingOriginator, ObjectHit) == false || effect.TargetInRange(meEntity, Projectile) == false) {

                // log that ability could not activate 
              effect.LogEffectText(meEntity, EffectAction.NoEffect);
            ...
            ActiveEffect activeEffect = new ActiveEffect(effect, AbilityHitPoint, AbilityName);
            ...
            if (this.editorRecordActiveEffectHistory)
                this.editorActiveEffectHistory.Add(System.DateTime.Now + ": " + effect.effectName + " from " + effect.abilityName + " was activated on " + gameObject.name + " by " + AfflictingOriginator.gameObject.name);


            // activate effect and store the coroutine so we can interupt the activation later if required
            StartCoroutine(activeEffect.ActivateCoroutine = activeEffect.effect.Activate(meEntity, AfflictingOriginator, AbilityType, activeEffect.hitPoint, Projectile));

            AfflictingOriginator.AddToDiagnosticLog(effect.abilityName + " " + effect.effectName + " effect is now active on " + gameObject.name);
    ...
    private void RemoveActiveEffect(ActiveEffect activeEffect, ABC_IEntity castingOriginator) {
        ...
        //run Deactivate code
        activeEffect.effect.Deactivate(meEntity, castingOriginator);
