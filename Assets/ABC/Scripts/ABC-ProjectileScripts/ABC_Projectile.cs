...
public class ABC_Projectile : MonoBehaviour {
    ...
    private bool CanBounce(GameObject CollidedObj) {
        if (ability.bounceMode == true && this.bounceCounter < ability.bounceAmount && ability.abilityType != AbilityType.Melee && (ability.startBounceTagRequired == false || ability.startBounceTagRequired == true && ABC_Utilities.ObjectHasTag(CollidedObj, ability.startBounceRequiredTags)))
            return true;
        else
            return false;
        ...
        foreach (Collider item in col) {
            ...
            switch (ability.bounceTarget) {
                ...
                case BounceTarget.NearestTag:
                    ...
                    if (ABC_Utilities.ObjectHasTag(item.gameObject, ability.bounceTag))
                        newBounceTarget = item.transform.gameObject;
    ...
    private bool CorrectCollision(GameObject Obj, CollisionType Type) {
        ...
        if (ability.abilityIgnoreTag.Count > 0 && ABC_Utilities.ObjectHasTag(Obj, ability.abilityIgnoreTag) == true) 
            return false;


        // if the object does not have a tag matching our required tags then this is not the correct collision so return false 
        if (ability.abilityRequiredTag.Count > 0 && ABC_Utilities.ObjectHasTag(Obj, ability.abilityRequiredTag) == false)
            return false;
    ...
    private void CollideHandler(GameObject CollidedObj, Vector3 HitPoint = default(Vector3), bool Destroy = true, bool Bounce = true) {
        ...
        if (ability.modifyGameSpeedOnImpact == true)
           originator.ModifyGameSpeed(ability.modifyGameSpeedOnImpactSpeedFactor, ability.modifyGameSpeedOnImpactDuration);

        //Shake camera 
        if (ability.shakeCameraOnImpact == true)
            originator.ShakeCamera(ability.shakeCameraOnImpactDuration, ability.shakeCameraOnImpactAmount, ability.shakeCameraOnImpactSpeed);
        ...
        if (Bounce && this.CanBounce(CollidedObj)) {
            ...
            if (ability.attachToObjectOnImpact == true && this.gameObject.transform.parent == null && ABC_Utilities.DiceRoll(ability.attachToObjectProbabilityMinValue, ability.attachToObjectProbabilityMaxValue) == true)
            {
                ...
                if (Destroy == true && ABC_Utilities.ObjectHasTag(CollidedObj, ability.destroyIgnoreTag) == false)
                    StartCoroutine(DestroyProjectile(ability.destroyDelay, CollidedObj));
            }
        }
    ...
    private void DisableProjectile() {
        ...
        ability.DisableProjectile(meTransform.gameObject, originator);
    ...
    private IEnumerator SplashMe(GameObject IgnoredObject = null) {
        ...
        if (ability.destroySplashExplosion == true) {
            ...
            foreach (ABC_IEntity entity in ABC_Utilities.GetAllABCEntitiesInRange(meTransform.position, ability.destroySplashExplosionRadius).ToList()) {
                ...
                if (ability.destroySplashExplosionTagLimit == true && ABC_Utilities.ObjectHasTag(entity.gameObject, ability.destroySplashExplosionAffectTag) == false)
                    continue;
    ...
    void OnCollisionStay(Collision col) {
        ...
        //Run method to add effects if the collision is a projectile
        this.ActivateCollision(col.gameObject, CollisionType.OnStay, col.contacts.FirstOrDefault().point);
    ...
    void OnTriggerStay(Collider col) {
        ...
        //Run method to add effects if the collision is a projectile
        this.ActivateCollision(col.gameObject, CollisionType.OnStay, meTransform.position);
    ...
    void OnEnable() {
        ...
        // Destroy object after chosen seconds - if 0 then the duration is unlimited
        if (ability.duration > 0)
            StartCoroutine(DestroyProjectile(ability.duration));
